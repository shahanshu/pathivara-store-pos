...
export const GET = withAdminAuth(async function GET(request, context) {
    ...
    // CRITICAL FIX: Read the parameter matching the directory name "[importerid]"
    const importerId = params?.importerid; 

    if (!importerId) {
        return NextResponse.json({ success: false, message: 'Importer ID (importerid) not found in parameters' }, { status: 400 });
    }
...
export const DELETE = withAdminAuth(async function DELETE(request, context) {
    ...
    if (!importerId) {
        return NextResponse.json({ success: false, message: 'Importer ID (from importerid param) not found in parameters.' }, { status: 400 });
    }
    if (!ObjectId.isValid(importerId)) {
        return NextResponse.json({ success: false, message: 'Invalid Importer ID format.' }, { status: 400 });
    }
